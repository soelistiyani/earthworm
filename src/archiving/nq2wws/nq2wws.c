 ...
 *   nq2wws.c                                                            *
 *                                                                       *
 *   Program to read miniSEED files from NetQuakes and to export them    *
 *   over a socket to a Winston Waveserver.                              *
 *                                                                       *
 *   This is not the best way to do this because the rate of delivery of *
 *   tracebufs to Winston is artificially throttled to prevent overruns. *
 *   If possible, you should use nq2wws_ack.                             *
 *                                                                       *
...
static char    ServerIPAdr[64];        /* server's IP address               */
static char    ServerPort[16];         /* Server's well-known port number   */
...
int main( int argc, char **argv )
{
	/* Socket variables: */
	int    clientLen;
	char   client_ip[64];        /* IP address of client from inet_ntoa   */
	char   client_port[10];
   ...
   PassiveSocket = setuptcpserver_ew(ServerIPAdr, ServerPort, 0);

	if ( PassiveSocket < 0 ) {
		logit( "et", "%s(%s): Error setting up TCP server socket; exiting!\n", Argv0, MyModName );
		tport_detach( &Region );
		return( -1 );
	}
	SockStatus = SOCK_NEW;
   ...
   formatsocketaddress_ew((struct sockaddr *) &client, clientLen, client_ip, client_port);
...
/*5*/
				...
				if(str) {
					...
					strncpy( ServerIPAdr, str, sizeof( ServerIPAdr ) );
				}
				init[5] = 1;
            }
...
/*6*/
				...
				if((str = k_str()) != NULL) {
					strncpy( ServerPort, str, sizeof( ServerPort ));
					init[6] = 1;
				}
            }
