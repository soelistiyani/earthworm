...
#define ncommand 9          /* Number of commands in the config file */
...
int GetConfig( char *config_file, GPARM *Gparm )
{
	...
	***************************************************/
	...
	Gparm->CutOffFreq = 0.3; /* KMA */
	Gparm->ProduceOldStylePick = 0;
	...
	while ( nfiles > 0 )          /* While there are config files open */
	{
		while ( k_rd() )           /* Read next line from active file  */
		{
			...
			*****************************/
			...
			/*opt*/ else if ( k_its( "CutOffFreq" ) )
			{
				Gparm->CutOffFreq = k_val();
			}
			/*opt*/ else if ( k_its( "ProduceOldStylePick" ) )
			{
				Gparm->ProduceOldStylePick = k_int();
			}
			...
			/*opt*/ else if ( k_its( "GetLogo" ) )
			{
				...
				if( tlogo == NULL )
				{
					logit( "e", "pick_FP: GetLogo: error reallocing %zu bytes.\n",
