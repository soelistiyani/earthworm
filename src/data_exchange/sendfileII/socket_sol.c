 ...
 *
 *    $Id: socket_sol.c 7480 2018-06-22 23:58:59Z baker $
 *
 *    Revision history:
 *     $Log: socket_sol.c,v $
 *     Revision 1.4  2010/04/27 18:43:41  paulf
...
int ConnectToGetfile( void )
{
     ...
     ***************************/
    if (socket_open) {
	log_msg("et", "logic error: socket already open\n");
    } else {
	sd = socket( AF_INET, SOCK_STREAM, 0 );
	if ( sd == -1 )
	{
	    log_msg( "et", "socket() error: %s\n", strerror(errno) );
    ...
    if ( address == -1 )
    {
	log_msg( "et", "Bad server IP address: %s  Exiting.\n", ServerIP );
    ...
    ***************************************************/
    log_msg( "e", "\n" );
    log_msg( "et", "Connecting to %s  port %d\n", ServerIP, ServerPort );
    if ( connect( sd, (struct sockaddr *)&server, sizeof(server) ) == -1 )
    {
	log_msg( "et", "connect() error: %s\n", strerror(errno) );
	CloseSocketConnection();
	return SENDFILE_FAILURE;
    }
    ...
    {
	log_msg( "et", "ioctl() FIONBIO error: %s\n", strerror(errno) );
	CloseSocketConnection();
	return SENDFILE_FAILURE;
    }
...
static int Send_all( int sock, char buf[], int nbytes )
{
	...
	****************************************************/
	...
	if ( selrc == -1 )
	{
	    log_msg( "et", "select() error: %s", strerror(errno) );
	    return -1;
	}
	if ( selrc == 0 )
	{
	    log_msg( "et", "select() timed out\n" );
	    return -1;
	}
	if ( FD_ISSET( sock, &writefds ) )    /* A writeable event occurred */
	{
	    ...
	    ******************************************/
	    if ( getsockopt( sock, SOL_SOCKET, SO_ERROR, (char *)&sockerr,
			     &optlen ) == -1 )
	    {
		log_msg( "et", "getsockopt() error: %s", strerror(errno) );
		return -1;
	    }
	    if ( sockerr != 0 )
	    {
		log_msg( "et", "Error detected by getsockopt(): %s\n",
                     strerror(sockerr) );
		return -1;
	    }
	    ...
	    *****************************************************/
		...
		{
		    log_msg( "et", "send() would block\n" );
		    continue;
		}
		else
		{
		    log_msg( "et", "send() error: %d", strerror(errno) );
		    return -1;
		}
	    }
	    if ( rc == 0 )           /* Shouldn't see this */
	    {
		log_msg( "et", "Error: send() == 0\n" );
		return -1;
	    }
	    nwritten += rc;
	    nBytesToWrite -= rc;
...
int SendBlockToSocket( char *buf, int blockSize )
{
    ...
    if ( blockSize > 999999 )
    {
	log_msg( "et", "Error. Block is too large to send.\n" );
	return SENDFILE_FAILURE;
    }
	...
	****************************************************/
	...
	if ( FD_ISSET( sock, &readfds ) )    /* A readable event occurred */
	{
	    ...
	    ****************************************************/
		...
		{
		    log_msg( "et", "recv() error: %s", strerror(errno) );
		    return -1;
		}
	    }
	    if ( rc == 0 )
	    {
		log_msg( "et", "Error. recv()==0\n" );
		return -1;
	    }
	    nread += rc;
	    nBytesToRead -= rc;
...
int GetAckFromSocket( int timeout )
{
    ...
    if ( strcmp( Ack, "ACK" ) == 0 )
    {
	log_msg( "et", "ACK received\n" );
	return SENDFILE_SUCCESS;
    }

    log_msg( "et", "Bad ACK string received: %s\n", Ack);
...
void CloseSocketConnection( void )
{
	...
	ret = close( sd );
	if (ret != 0)
	    log_msg( "et", "Error closing socket: %s\n", strerror(errno));
	socket_open = 0;
    }
    return;
}
