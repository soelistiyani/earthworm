/* $Id: grf.c 7290 2018-04-07 21:45:50Z baker $ */
/*-----------------------------------------------------------------------
...
BOOL GRFReadFile(FILE *file, GRF_PACKET * packet)
{
	...
	if (length < GRF_HDR_SIZE) {
		logit("et", "grf2ew: ERROR: GRFReadFile: Packet length error: too short: %u < %zu\n", length, GRF_HDR_SIZE);
		return FALSE;
	}
...
BOOL GRFReadSocket(SOCKET socket, GRF_PACKET * packet, USTIME timeout)
{
	...
	length = ntohs(*((UINT16 *)&packet->serial.bytes[4]));
	if (length < GRF_HDR_SIZE) {
		logit("et", "grf2ew: ERROR: GRFReadSocket: Packet length error: too short: %u < %zu\n", length, GRF_HDR_SIZE);
		return FALSE;
	}
	if (length > GRF_MAX_PACKET_LEN) {
