...
{
	if( ! msr )  {
		ms_log (2, "msr_init(): No msr pointer\n");
		return NULL;
	}
...
void msr_reset( MSRecord *msr )
{
	msr->nextBlktBlk = msr->nextBlockData = 0;
}
  ...
  if ( blkt )
    {
      if ( chainpos != 0 )
	{
	  blkt = &msr->blktLink[ msr->nextBlktBlk++ ];
      ...
      else
	{
	  ...
	  blkt->next = &msr->blktLink[ msr->nextBlktBlk++ ];;
    ...
    {
      msr->blkts = &msr->blktLink[ msr->nextBlktBlk++ ];
      ...
      if ( msr->blkts == NULL )
	{
  	...
  	blkt->blktdata = &msr->msBlockData[ msr->nextBlockData ];
  	msr->nextBlockData += length;
...
int msr_normalize_header( MSRecord *msr, flag verbose )
{
	...
	if( msr->winsdrMode == 1 )
	    msr->fsdh.reserved = msr->station[ 5 ];
	else
	    msr->fsdh.reserved = ' ';
