  ...
  int           verbose)
{
  ...
  *pxn = XN;
  ...
  if ( verbose > 2 )
    ms_log( 1, "forward/reverse integration constants:\nX0: %d  XN: %d\n", *px0, *pxn);
	  ...
	  switch (compflag)
		...
		*diff++ = pf->w[wn].byte[i];
	    ...
	    default:
	      ...
	      ms_log (2, "msr_unpack_steim1: invalid compression flag = %d\n", compflag);
  ...
  if ( nd != num_samples )
    {
      ms_log (1, "Warning: msr_unpack_steim1: number of samples indicated in header (%d) does not equal data (%d)\n",
	      num_samples, nd);
    }
  ...
  if (last_data != *pxn)
    {
      ms_log (1, "Warning: Data integrity check for Steim-1 failed, last_data=%d, xn=%d\n", last_data, *pxn);
    }
  ...
  int           verbose)
{
  ...
  *pxn = XN;
  ...
  if ( verbose > 2 )
    ms_log (1, "forward/reverse integration constants:  X0: %d  XN: %d\n", *px0, *pxn);
	  ...
	  switch (compflag)
		...
		*diff++ = pf->w[wn].byte[i];
		...
		default:	/*  should NEVER get here  */
		  ms_log (2, "msr_unpack_steim2: invalid compflag, dnib, fn, wn = %d, %d, %d, %d\n", compflag, dnib, fn, wn);
		  return MS_STBADCOMPFLAG;
		}
	    ...
	    default:
	      /* Should NEVER get here */
	      ms_log (2, "msr_unpack_steim2: invalid compflag, fn, wn = %d, %d, %d - nsamp: %d\n", compflag, fn, wn, nd);
	      return MS_STBADCOMPFLAG;
	    }
	}
      ++pf;
  ...
  if ( nd != num_samples )
    {
      ms_log (1, "Warning: msr_unpack_steim2: number of samples indicated in header (%d) does not equal data (%d)\n",
	      num_samples, nd);
    }
  ...
  if (last_data != *pxn)
    {
      ms_log (1, "Warning: Data integrity check for Steim-2 failed, last_data=%d, xn=%d\n",
	      last_data, *pxn);
    }
  ...
  int		swapflag)	/* if data should be swapped.	        */
{
  ...
  if ( encoding != DE_GEOSCOPE24 &&
    ...
    {
      ms_log (2, "msr_unpack_geoscope: unrecognized GEOSCOPE encoding: %d\n", encoding);
      return -1;
    }
  ...
  int		swapflag)	/* if data should be swapped.	        */
{
  ...
  for (nd=0; nd<req_samples && nd<num_samples; nd++)
	...
	{
	  ms_log (2, "msr_unpack_sro: SRO gain ranging exponent out of range: %d\n", exponent);
	  return MS_GENERROR;
	}
